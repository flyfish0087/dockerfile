...
def collect_static():
    ...
    os.chdir(os.path.join(BASE_DIR, 'apps'))
    subprocess.call('python manage.py collectstatic --no-input -v 0', shell=True)

def start_settings():
    print("- Start setting")
    os.chdir(os.path.join(BASE_DIR, 'apps'))
    subprocess.call('sed -i "s/DEBUG = CONFIG.DEBUG or False/DEBUG = $DEBUG/" jumpserver/settings.py', shell=True)
    subprocess.call('''sed -i "s@'DEBUG',@'INFO',@" jumpserver/settings.py''', shell=True)
